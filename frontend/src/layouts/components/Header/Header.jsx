            ...
            {account &&
                <div ref={searchRef} className={"relative lg:flex justify-center hidden w-[550px]"}>
                    ...
                    {showDropdown && searchTerm && (
                        <div
                            className="absolute top-[60px] left-0 right-0 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50 text-black">
                                        ...
                                        <li key={product.id}>
                                            ...
                                            >
                                                ...
                                                <div className="flex flex-col">
                                                    <span
                                                        className="font-semibold text-sm text-gray-900 line-clamp-1">{product.name}</span>
                                                    <span
                                                        className="text-blue-600 font-bold text-sm">{product.price.toLocaleString('vi-VN')} ₫</span>
                                                </div>
                                            </div>
                                        </li>
                                    ))}
            ...
            <div className={"md:flex lg:justify-end lg:min-w-[100px] text-[16px]"}>
                {account?.fullName && (
                    <div className="flex items-center gap-4 font-medium text-gray-200">
                        <div
                            className="flex items-center gap-3 bg-gray-800/50 hover:bg-gray-800 py-1.5 pl-1.5 pr-4 rounded-full border border-gray-700/50 transition-all cursor-pointer group"
                            onClick={() => navigate("/profile")}>
                            {account.imgUrl ? (
                                <img src={account.imgUrl} alt="Avatar"
                                     className="w-8 h-8 rounded-full object-cover border border-gray-600 group-hover:border-blue-500 transition-colors"/>
                            ) : (
                                <div
                                    className="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center border border-gray-600 group-hover:border-blue-500 transition-colors">
                                    <FaRegUser size={14} className="text-gray-400"/>
                                </div>
                            )}
                            <div className="hidden lg:flex flex-col text-left">
                                <span
                                    className="text-[10px] text-gray-400 font-bold uppercase tracking-tight leading-none">Account</span>
                                <span
                                    className="text-white text-sm font-bold leading-tight truncate max-w-[100px]">{account.fullName}</span>
                            </div>
                        </div>

                        <Link to={"/"} onClick={handleLogOut} title="Log out">
                            <FiLogOut
                                className="text-red-400 hover:text-red-500 h-6 w-6 hover:bg-red-500/10 p-1 rounded-full transition-all"/>
                        </Link>
                    </div>
                )}
                ...
                <div className="md:flex md:justify-end lg:items-center gap-6 ml-4">
                    ...
                    {
                        account && (<div className={"relative cursor-pointer hover:text-blue-400 transition-colors"}
