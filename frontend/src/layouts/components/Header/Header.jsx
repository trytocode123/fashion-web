            ...
            >
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col justify-center group">
                        <Link to="/about" className="flex items-baseline gap-1 group">
                            <span
                                className="font-black text-2xl tracking-tighter group-hover:text-indigo-400 transition-colors">FASHION</span>
                            <span
                                className="text-zinc-500 font-medium text-sm group-hover:text-indigo-300 transition-colors">hub</span>
                        </Link>
                    </div>
                </div>
                ...
                {account && (
                    <div ref={searchRef} className="relative hidden md:flex items-center flex-1 max-w-lg mx-8 group">
                        <div
                            className="absolute left-4 z-10 text-zinc-500 group-focus-within:text-indigo-400 transition-colors">
                            <FaSearch size={14}/>
                        </div>
                        <input
                                    ...
                                    <ul className="p-2 space-y-1">
                                        {searchResults.map((product) => (
                                            <li key={product.id}>
                                                ...
                                                >
                                                    <div
                                                        className="w-12 h-12 rounded-lg overflow-hidden bg-zinc-800 flex-shrink-0 border border-zinc-700">
                                                        <img src={product.img} alt={product.name}
                                                             className="w-full h-full object-cover"/>
                                                    </div>
                                                    <div className="flex flex-col">
                                                        <span
                                                            className="font-bold text-sm text-zinc-100 line-clamp-1">{product.name}</span>
                                                        <span
                                                            className="text-indigo-400 font-black text-xs uppercase tracking-wider">
                ...
                <div className="flex items-center gap-3 md:gap-6">
                    {account && (
                        <div className="hidden md:flex items-center gap-6 text-zinc-400">
                            ...
                            <div
                                className="relative group cursor-pointer hover:text-indigo-400 transition-all duration-300 hover:scale-110"
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                >
                                    {account.imgUrl ? (
                                        <img src={account.imgUrl} alt="Avatar"
                                             className="w-8 h-8 rounded-full object-cover ring-1 ring-zinc-700 group-hover:ring-indigo-500"/>
                                    ) : (
                                        <div
                                            className="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center border border-zinc-700 group-hover:border-indigo-500">
                                            <FaRegUser size={12} className="text-zinc-500 group-hover:text-indigo-400"/>
                                        </div>
                                    )}
                                    <div className="hidden lg:flex flex-col">
                                        <span
                                            className="text-zinc-200 text-xs font-black truncate max-w-[80px]">{account.fullName?.split(' ').pop()}</span>
                                    </div>
                                </div>
                    ...
                    {account && (
                        <div className="md:hidden relative cursor-pointer text-zinc-400"
                             onClick={() => navigate("/cart")}>
                            <FaCartShopping size={20}/>
                            <span
                                className="absolute -top-2 -right-2 min-w-[16px] h-4 rounded-full bg-indigo-600 text-white font-black text-[8px] flex items-center justify-center border border-zinc-950">
                                {totalCartItems}
                            </span>
                        </div>
                    )}
                    ...
                    <nav className="flex flex-col gap-8 flex-1">
                        {[
                            ...
                            >
                                <span>{item.name}</span>
                                <div
                                    className="w-0 h-0.5 bg-indigo-500 group-hover:w-8 transition-all duration-300"></div>
                            </Link>
                        ))}
                    </nav>
                    ...
                    <div className="mt-auto pt-8 border-t border-zinc-900 flex flex-col gap-6">
                        {account ? (
                            <div className="flex items-center gap-4 p-4 rounded-2xl bg-zinc-900 border border-zinc-800">
                                <img src={account.imgUrl || "https://via.placeholder.com/40x40"} alt="User"
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <span
                                        className="text-[10px] text-zinc-500 uppercase tracking-widest">{account.username}</span>
                                </div>
                            ...
                            <Link to="/login" onClick={() => setIsMenuOpen(false)}
                                  className="bg-white text-black py-4 rounded-2xl text-center font-black uppercase tracking-widest text-xs shadow-xl active:scale-95 transition-all">
                                Get Started
                            </Link>
                        )}
                        <p className="text-[9px] text-zinc-600 font-bold uppercase tracking-[0.3em] text-center">Fashion
                            Hub • Premium Quality</p>
                    </div>
                </div>
            </div>
        </>
